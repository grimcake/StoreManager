...
void AuditorSalesTab::on_pushButton_clicked()
{
    ...
    QString OUTNUMBER = model->data(model->index(rowIndex, 3)).toString();
    QString QHSL = model->data(model->index(rowIndex, 5)).toString();
    //qDebug()<<str<<endl;
    QDateTime current_time = QDateTime::currentDateTime();
    QString SHDATE = current_time.toString("yyyyMMddhhmmss");

    QSqlQuery query;
    QString sqlsentence("update T_CHXX set SFSH = 1, SHDATE = '"+SHDATE+"', QHSL = "+QString::number(QHSL.toInt())+" where OUTID = '"+OUTID+"';");
...
void AuditorSalesTab::on_pushButton_2_clicked()
{
    DBmodule *dbmodule = DBmodule::getDBmodule();
    dbmodule->connect();
    int rowIndex = ui->tableView->currentIndex().row();
    ...
    QSqlQuery query;
    QString sqlsentence("update T_CHXX set SFSH = 2, SHDATE = '"+SHDATE+"' where OUTID = '"+OUTID+"';");
    qDebug()<<sqlsentence<<endl;
    bool ret = query.exec(sqlsentence);
    if(ret == false){
        qDebug()<<"update error"<<endl;
